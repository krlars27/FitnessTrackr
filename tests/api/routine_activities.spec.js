...
describe("/api/routine_activities", () => {
  describe("PATCH /api/routine_activities/:routineActivityId (**)", () => {
    it("Updates the count or duration on the routine activity", async () => {
      ...
      const fakeRoutine = await createFakePublicRoutine(fakeUser.id, "On Saturday", "Until I'm stronger")
      const fakeActivity = await createFakeActivity("Free Weights", "Lift all the things")
      ...
      expect(response.body).toEqual(
        objectContaining(updatedRoutineActivityData)
      )
    })
    ...
    it("should return an error if the owner of the routine isn't the one trying to edit it", async () => {
      ...
      const fakeRoutine = await createFakePublicRoutine(fakeUser.id, "In the evening", "until I lose 5 pounds")
      const fakeActivity = await createFakeActivity("Jump Rope", "Just like we did on the school yard")
      const fakeRoutineActivity = await createFakeRoutineActivity(
        fakeRoutine.id,
        fakeActivity.id
      )
      ...
      const { token: secondUserToken } = await createFakeUserWithToken("Lizzy")
  ...
  describe("DELETE /api/routine_activities/:routineActivityId (**)", () => {
    it("Removes an activity from a routine, uses hard delete", async () => {
      ...
      const fakeRoutine = await createFakePublicRoutine(fakeUser.id, "In the morning", "Early bird gets the worm")
      const fakeActivity = await createFakeActivity("Walk 10 miles", "All the walking")
      const fakeRoutineActivity = await createFakeRoutineActivity(
        fakeRoutine.id,
        fakeActivity.id,
      )
    ...
    it("Logged in user should be the owner of the modified object.", async () => {
      ...
      const fakeRoutine = await createFakePublicRoutine(fakeUser.id, "In the afternoon", "Until Charlie Brown gives up")
      const fakeActivity = await createFakeActivity("Football Kicking", "Make sure you pull it away quick")
      const fakeRoutineActivity = await createFakeRoutineActivity(
        fakeRoutine.id,
        fakeActivity.id,
      )
      ...
      // Create a second user
      const { token: secondUserToken } = await createFakeUserWithToken("Charlie")
